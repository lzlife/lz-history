...
export default function HistoryCmp({ enterAction, onGoToSetting }: HistoryCmpProps) {
  ...
  useEffect(() => {
    const order = JSON.parse(window.ztools.dbStorage.getItem('historyOrder') || '["edge","tabbit"]')
    const configured = order.filter((id: string) => window.ztools.dbStorage.getItem(`${id}HistoryPath`))
    if (configured.length === 0) {
      setError('请先在设置中配置历史记录文件路径')
      setLoading(false)
      return
    }
  ...
  const filtered = useMemo(() => {
    ...
    return history.filter(
      (h) => h.title.toLowerCase().includes(kw) || h.url.toLowerCase().includes(kw)
    )
  }, [history, search])
  ...
  useEffect(() => {
    ...
    reset()
    const historyPath = window.ztools.dbStorage.getItem(`${activeBrowser}HistoryPath`)
    if (!historyPath) {
      setError('请先在设置中配置历史记录文件路径')
      setLoading(false)
      return
    ...
    setSearch('')
    try {
      const result = window.services.readHistory(historyPath)
      setHistory(result)
      setError('')
    } catch (err: any) {
    ...
    return (
      <Empty>
        <EmptyHeader>
          ...
          <EmptyTitle>{search ? '未找到匹配的记录' : '暂无历史记录'}</EmptyTitle>
          <EmptyDescription>
            {search ? '尝试其他关键词搜索' : '在浏览器中浏览网页后会出现在这里'}
          </EmptyDescription>
        </EmptyHeader>
      </Empty>
    )
